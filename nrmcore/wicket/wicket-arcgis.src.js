...
Wkt.Wkt.prototype.deconstruct = function (obj) {
    ...
    if (geom.type === "polygon") {
        ...
        rings = [];
        for (i = 0; i < geom.rings.length; i += 1) {
            verts = [];

            for (j = 0; j < geom.rings[i].length; j += 1) {
                verts.push({
                    x: geom.rings[i][j][0], // First item is longitude, second is latitude
                    y: geom.rings[i][j][1]
                });
            }

            if (i > 0) {
                if (Wkt.isInnerRingOf(verts, rings[i - 1], geom.spatialReference)) {
