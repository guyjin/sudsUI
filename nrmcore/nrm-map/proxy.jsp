...
private String getNewTokenIfCredentialsAreSpecified(ServerUrl su, String url) throws IOException{
    ...
    if (isUserLogin || isAppLogin) {
        log.info("Matching credentials found in configuration file. OAuth 2.0 mode: " + isAppLogin);
        if (isAppLogin) {
            ...
            log.info("Service is secured by " + su.getOAuth2Endpoint() + ": getting new token...");
            ...
            if (infoUrl != "") {
                ...
                log.info("[Info]: Querying security endpoint...");

                String tokenServiceUri = su.getTokenServiceUri();

                if (tokenServiceUri == null || tokenServiceUri.isEmpty()){
                    log.info("Token URL not cached.  Querying rest info page...");
                 ...
                 if (tokenServiceUri != null & !tokenServiceUri.isEmpty()){
                    log.info("[Info]: Service is secured by " + tokenServiceUri + ": getting new token...");
...
private String exchangePortalTokenForServerToken(String portalToken, ServerUrl su) throws IOException{
    ...
    log.info("[Info]: Exchanging Portal token for Server-specific token for " + url + "...");
...
private String extractToken(String tokenResponse, String key) {
    ...
    if (token == null || token.isEmpty()) {
        log.warn("Token cannot be obtained: " + tokenResponse);
    } else {
...
*/
    ...
    //writing Log file
    private static final Logger log = LoggerFactory.getLogger("us.fed.fs.nrm.nrmcore.nrmmap.proxy");
...
log.info("Creating request for: " + uri);
ServerUrl serverUrl;
...
try {
    try {
        ...
        if (allowedReferers != null && allowedReferers.length > 0 && request.getHeader("referer") != null){
            ...
            if (!allowed){
                log.warn("Proxy is being used from an unsupported referer: " + request.getHeader("referer"));
        ...
        log.warn("Proxy is being used for an unsupported service: " + uri);
    ...
    if (!passThrough && serverUrl.getRateLimit() > -1) {
        synchronized(_rateMapLock){
            ...
            if (!rate.click()) {
                log.warn("Pair " + key + " is throttled to " + serverUrl.getRateLimit() + " requests per " + serverUrl.getRateLimitPeriod() + " minute(s). Come back later.");
	...
	try {
        ...
        log.error("There was an error sending a response to the client.  Will not try again.", finalErr);
    }
} catch (IOException e){
    try {
        log.error("A fatal proxy error occurred.",e);
