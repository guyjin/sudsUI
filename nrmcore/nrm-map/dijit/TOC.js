 ...
 * @author: Nianwei Liu
 * @fileoverview
 * <p>A TOC (Table of Contents) widget for ESRI ArcGIS Server JavaScript API.</p>
 * @see module:nrm-map/dijit/TOC
 */
/**
 * @module nrm-map/dijit/TOC
 * 
 */

// change log: 
// 2015-05-28: Added some unique IDs to help in selecting TOC elements, fixed slider title tag
// 2015-01-22: Use attribute field alias if available
// 2015-01-15: Added description property and more hover text logic
...
"dojo/dom-attr",
 ...
 "esri/request", "esri/domUtils", "dojo/_base/sniff"], function(
  declare, 
  has,
  aspect,
  lang,
  ...
  ArcGISTiledMapServiceLayer, esriRequest, esri){
  ...
  var _TOCNode = declare([_Widget, _Templated],{
    ...
    postCreate: function(){
      ...
      if (!this._noCheckNode) {
        ...
        var checked = (this._checkboxlabel === undefined) ? this.data.visible : true,
            title = 'Toggle visibility for ' + ((this._checkboxlabel === undefined) ? this.rootLayerTOC.config.title : this._checkboxlabel);
        if (dijit.form && dijit.form.CheckBox) {
          ...
          chk = domConstruct.create('input', {
            type: 'checkbox',
            checked: (this._checkboxlabel === undefined) ? this.data.visible : true
            ,style: 'margin: 7px 5px 0px 0px}' //artf11682 {margin: top right bottom left}
            //,title: 'Toggle visibility for ' + this.rootLayerTOC.config.title
            ,title: title
          }, this.checkContainerNode);
        }
        this.checkNode = chk;
        if (this._checkboxlabel) {
            domAttr.set(this.checkNode, 'class', 'agsjsTOCLegendCheckbox');
            domAttr.set(this.checkNode, 'id', 'tocLegendCheckbox-' + this.rootLayerTOC.id);
        } else {
            domAttr.set(this.checkNode, 'id', 'tocCheckbox-' + this.rootLayerTOC.id);
        }
      }
    ...
    _createRootLayerNode: function(rootLayer){
      ...
      this.labelNode.id = 'tocLabel-' + this.rootLayerTOC.id;
      domAttr.set(this.labelNode, 'tabindex', 0);
      //console.log('TOC label ID ' + this.labelNode.id, this);
      var title = this.rootLayerTOC.config.title;
      //console.log("TOC title, rootLayerTOC, rootLayer", title, this.rootLayerTOC, rootLayer);
      ...
      if (this.rootLayerTOC.config.slider) {
        this.sliderNode = domConstruct.create('div', {
          ...
          ,'id': 'tocSlider-' + this.rootLayerTOC.id
          ,'title': 'Adjust transparency for ' + this.rootLayerTOC.config.title
        }, this.rowNode, 'last');//
        var me = this;
        require(["dijit/form/HorizontalSlider", "dojo/domReady!"], function(HorizontalSlider){
					me.slider = new HorizontalSlider({
                        id: 'tocHorizontalSlider-' + me.rootLayerTOC.id,
						showButtons: false,
						...
						onChange: function(value){
							rootLayer.setOpacity(value / 100);
                                                        //console.log("New opacity: " + rootLayer.opacity.toString());
						},
						layoutAlign: 'right'
					});
      ...
      if (!this.rootLayerTOC.config.noLegend) {
        if (rootLayer._tocInfos) {
          ...
          if (r.infos) {
            ...
            if (r.rendererOpts && r.rendererOpts.fieldNameAlias) // ebodin 1/23/2015. For a renderer set up by nrm featureLayerView.
                af = r.rendererOpts.fieldNameAlias;              // This could be more complex, but only supports one fieldname for now.
      ...
      domAttr.set(this.iconNode, 'id','tocIcon-' + this.rootLayerTOC.id);
      this.labelNode.innerHTML = title;
      //dojo .attr(this.rowNode, 'title', title);
	  //domAttr.set(this.rowNode, 'title', title);
          // set altTitle for hover text (ebodin 1/15/2015)
          var altTitle = title; // default
          if (this.rootLayerTOC.config.description) {
              altTitle = title + ' (' + this.rootLayerTOC.config.description + ')';
          } else if (rootLayer.geometryType) {
              altTitle = title + ' (' + rootLayer.geometryType.replace('esriGeometry','').toLowerCase().replace('polyline','line') + ')';
          } else if (rootLayer.url) {
              altTitle += rootLayer.getTileUrl ? " (tiled map service)" : " (web map service)";
          }
          domAttr.set(this.rowNode, 'title', altTitle);
    },
    ...
    _createServiceLayerNode: function(serviceLayer){
      ...
      var altTitle = serviceLayer.name; // default
      domAttr.set(this.rowNode, 'title', altTitle);
    },
    /*
    ...
    _createLegendNode: function(rendLeg){
      if (this.rootLayer.url) this._noCheckNode = true;   //symbol checkbox
      domConstruct.destroy(this.containerNode);
      ...
      this._checkboxlabel = label; //symbol checkbox
      //console.log('_createLegendNode checkbox ' + label);
      ...
      if (!src) {
        if (rendLeg.symbol) {
          ...
          if (rendLeg.symbol.width && rendLeg.symbol.height) {
            // ebodin 1/27/2015 30 seems to be a good value
                w = 30; //Math.min(Math.max(w, 30), 50);
                h = 30; //Math.min(Math.max(h, 30), 50);
    ...
    _adjustToState: function(){
      if (this.checkNode) {
        ...
        var checked;
        if (this.checkNode.id.indexOf('tocLegendCheckbox') === 0) {
            var label = this.checkNode.title.replace('Toggle visibility for ', ''),
                renderer = this.rootLayer.renderer,
                info;
            for (var i = 0; i < (renderer ? renderer.infos.length : 0); i++) {
                info = renderer.infos[i];
                if (info.label === label) {
                    checked = info.symbol.color.a > 0;
                    break;
                }
            }
        } else {
            checked = this.legend ? this.legend.visible : this.serviceLayer ? this.serviceLayer.visible : this.rootLayer ? this.rootLayer.visible : false;
        }
        ...
        if (this.sliderNode) {
            this.sliderNode.hidden = !checked;
        }
      }
 ...
 var _RootLayerTOC = declare([_Widget], {
     ...
     * @constructor
     * @private
     * @name module:nrm-map/dijit/TOC~_RootLayerTOC
    ...
    _getLegendInfo: function(){
      ...
      if (this.rootLayer.version >= 10.01) {
        ...
        url = '//www.arcgis.com/sharing/tools/legend';
	 ...
	 * @event load
         * @private
         * @memberOf module:nrm-map/dijit/TOC~_RootLayerTOC
	 */
    onLoad: function(){
  ...
  var TOC = declare("agsjs.dijit.TOC", [_Widget, Evented],{
    ...
    /**
     * This is an object literal that specify the options for each map rootLayer layer.
     * @typedef {Object} module:nrm-map/dijit/TOC~TOCLayerInfo
     ...
     * @property {string} [description] value to show after title in hover text (defaults to geometry type or service description)
     *
     */
    /**
     * This is an object literal that specify the option to construct a {@link TOC}.
     * @typedef {Object} module:nrm-map/dijit/TOC~TOCOptions
     * @property {esri.Map} [map] the map instance. required.
     * @property {Object[]} [layerInfos] a subset of layers in the map to show in TOC. each object is a {@link module:nrm-map/dijit/TOC~TOCLayerInfo}
     * @property {Number} [indentSize] indent size of tree nodes. default to 18.
     */
    /** 
     * Create a Table Of Contents (TOC)
     * @alias module:nrm-map/dijit/TOC
     * @constructor
     * @class This class is a Table Of Content widget.
     * @param {module:nrm-map/dijit/TOC~TOCOptions} opts
